<template>
  <div>
    <h2 id="page-heading" data-cy="ProjectHeading">
      <span v-text="$t('jhip02App.project.home.title')" id="project-heading">Projects</span>
      <div class="d-flex justify-content-end">
        <button class="btn btn-info mr-2" v-on:click="handleSyncList" :disabled="isFetching">
          <font-awesome-icon icon="sync" :spin="isFetching"></font-awesome-icon>
          <span v-text="$t('jhip02App.project.home.refreshListLabel')">Refresh List</span>
        </button>
        <router-link :to="{ name: 'ProjectCreate' }" custom v-slot="{ navigate }">
          ...
          >
            ...
            <span v-text="$t('jhip02App.project.home.createLabel')"> Create a new Project </span>
          </button>
        </router-link>
      </div>
    </h2>
    ...
    <div class="alert alert-warning" v-if="!isFetching && projects && projects.length === 0">
      <span v-text="$t('jhip02App.project.home.notFound')">No projects found</span>
    </div>
      ...
      <table class="table table-striped" aria-describedby="projects">
        <thead>
          <tr>
            ...
            <th scope="row"><span v-text="$t('jhip02App.project.name')">Name</span></th>
            <th scope="row"><span v-text="$t('jhip02App.project.customerGroup')">Customer Group</span></th>
            <th scope="row"></th>
          </tr>
        </thead>
        <tbody>
          <tr v-for="project in projects" :key="project.id" data-cy="entityTable">
            ...
            <td>{{ project.name }}</td>
            <td>
              <div v-if="project.customerGroup">
                <router-link :to="{ name: 'CustomerGroupView', params: { customerGroupId: project.customerGroup.id } }">{{
                  project.customerGroup.id
                }}</router-link>
              </div>
            </td>
            <td class="text-right">
    ...
    <b-modal ref="removeEntity" id="removeEntity">
      <span slot="modal-title"
        ><span id="jhip02App.project.delete.question" data-cy="projectDeleteDialogHeading" v-text="$t('entity.delete.title')"
          >Confirm delete operation</span
        ></span
      >
      <div class="modal-body">
        <p id="jhi-delete-project-heading" v-text="$t('jhip02App.project.delete.question', { id: removeId })">
