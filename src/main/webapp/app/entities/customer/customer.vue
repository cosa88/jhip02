<template>
  <div>
    <h2 id="page-heading" data-cy="CustomerHeading">
      <span v-text="$t('jhip02App.customer.home.title')" id="customer-heading">Customers</span>
      <div class="d-flex justify-content-end">
        <button class="btn btn-info mr-2" v-on:click="handleSyncList" :disabled="isFetching">
          <font-awesome-icon icon="sync" :spin="isFetching"></font-awesome-icon>
          <span v-text="$t('jhip02App.customer.home.refreshListLabel')">Refresh List</span>
        </button>
        <router-link :to="{ name: 'CustomerCreate' }" custom v-slot="{ navigate }">
          ...
          >
            ...
            <span v-text="$t('jhip02App.customer.home.createLabel')"> Create a new Customer </span>
          </button>
        </router-link>
      </div>
    </h2>
    ...
    <div class="alert alert-warning" v-if="!isFetching && customers && customers.length === 0">
      <span v-text="$t('jhip02App.customer.home.notFound')">No customers found</span>
    </div>
      ...
      <table class="table table-striped" aria-describedby="customers">
        <thead>
          <tr>
            ...
            <th scope="row"><span v-text="$t('jhip02App.customer.name')">Name</span></th>
            <th scope="row"><span v-text="$t('jhip02App.customer.enName')">En Name</span></th>
            <th scope="row"><span v-text="$t('jhip02App.customer.fullName')">Full Name</span></th>
            <th scope="row"></th>
          </tr>
        </thead>
        <tbody>
    ...
    <b-modal ref="removeEntity" id="removeEntity">
      <span slot="modal-title"
        ><span id="jhip02App.customer.delete.question" data-cy="customerDeleteDialogHeading" v-text="$t('entity.delete.title')"
          >Confirm delete operation</span
        ></span
      >
      <div class="modal-body">
        <p id="jhi-delete-customer-heading" v-text="$t('jhip02App.customer.delete.question', { id: removeId })">
